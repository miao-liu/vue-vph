<template>
  <form>
      ...
      <p>
          ...
          <button @click="subForm" :disabled="is_disabled" :class="is_disabled ? 'sub' : 'sub abled'">登录</button>
      <div class="iterm">
    ...
    subForm (event) {
      ...
      ajax('post', this.sub_url, { username: this.uname, userpass: this.upass }).then(res => {
        ...
        if (res.code === 1) {
          ...
          this.$store.commit('user/setToken', (Math.random() * 16).toString('16').split('.')[1])
          window.sessionStorage.setItem('username', this.uname)
